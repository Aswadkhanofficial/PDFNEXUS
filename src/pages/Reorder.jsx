        ...
        {step === 'select' && !isLoading && (
          <div className="w-full flex flex-col gap-4">
            <div className="flex items-center justify-between gap-3">
              <span className="text-sm font-medium text-slate-300">{order.length} pages</span>
              <button
                type="button"
                onClick={resetOrder}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 max-h-[55vh] overflow-y-auto pr-1">
              {order.map((srcIndex, pos) => (
                <div
                  ...
                  className="flex flex-col items-center gap-2 rounded-xl border bg-slate-950/60 p-3 cursor-grab active:cursor-grabbing transition-colors border-slate-800 hover:border-purple-500"
                >
                  ...
                  <div className="flex items-center justify-between w-full gap-1">
                    <span className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-400">
                      ...
                      <span className={`flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-bold ${
                        pos === srcIndex ? 'bg-slate-700 text-slate-300' : 'bg-purple-600 text-white'
                      }`}>
                        {pos + 1}
                      </span>
                      P{srcIndex + 1}
