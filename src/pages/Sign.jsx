      ...
      <div className="max-w-4xl w-full bg-white border border-slate-200 rounded-2xl p-8 shadow-2xl dark:bg-slate-900 dark:border-slate-800">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">E-Signature Workspace</h1>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400">Upload your document, drag your signature into place, and download the signed PDF.</p>
          </div>
        </div>

        {/* File Upload Section */}
        <div className="mb-6">
          <label className="block text-sm font-medium text-slate-700 mb-2 dark:text-slate-300">1. Select PDF Document</label>
          <div className="border-2 border-dashed border-slate-200 hover:border-purple-500/50 rounded-xl p-5 text-center transition-colors cursor-pointer relative bg-slate-100/60 dark:border-slate-800 dark:bg-slate-950/50">
            <input type="file" accept="application/pdf" onChange={handleFileChange} className="absolute inset-0 opacity-0 cursor-pointer" />
            <FileUp className="w-7 h-7 text-purple-600 mx-auto mb-2 dark:text-purple-400" />
            {selectedFile ? (
              <p className="text-sm text-purple-700 font-semibold dark:text-purple-300">{selectedFile.name}</p>
            ) : (
              <>
                <p className="text-sm text-slate-700 dark:text-slate-300">Click to upload target PDF</p>
          ...
          {activeTab === 'draw' ? (
            <div>
              <div className="flex justify-between items-center mb-2">
                <span className="text-xs text-slate-600 dark:text-slate-400">Draw inside the box below - it will appear on the document instantly</span>
                <button onClick={handleClearPad} className="text-xs text-slate-600 hover:text-red-500 flex items-center gap-1 transition-colors dark:text-slate-400 dark:hover:text-red-400">
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-slate-200 hover:border-purple-500/50 rounded-xl p-6 text-center transition-colors cursor-pointer relative bg-slate-100/60 dark:border-slate-800 dark:bg-slate-950/50">
                ...
                <ImageIcon className="w-7 h-7 text-purple-600 mx-auto mb-2 dark:text-purple-400" />
                {sigImageFile ? (
                  <p className="text-sm text-purple-700 font-semibold dark:text-purple-300">{sigImageFile.name}</p>
                ) : (
                  <p className="text-sm text-slate-700 dark:text-slate-300">Click to upload signature image</p>
                )}
              </div>
            </div>
          )}
        ...
        {selectedFile && signatureDataUrl && (
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">3. Position Signature on the Document</label>
              {pdfMeta && (
                <span className="inline-flex items-center gap-1.5 text-xs text-slate-600 bg-slate-100/80 border border-slate-200 rounded-full px-2.5 py-1 dark:text-slate-400 dark:bg-slate-800/60 dark:border-slate-700">
                  <FileText className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" /> Page 1 of {pdfMeta.pageCount}
                </span>
              )}
            </div>
            <div className="bg-slate-100/70 border border-slate-200 rounded-2xl p-3 shadow-inner dark:bg-slate-950/60 dark:border-slate-800">
              ...
              <div className="flex items-center justify-between mt-2.5 px-1">
                ...
                <button
                  ...
                  className="text-xs text-slate-600 hover:text-purple-600 flex items-center gap-1.5 transition-colors dark:text-slate-400 dark:hover:text-purple-300"
                >
        ...
        <button
          ...
          className={`w-full disabled:opacity-50 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all ${
            paywall.isLocked
              ? 'bg-slate-200 text-purple-600 cursor-not-allowed shadow-none border border-slate-200 dark:bg-slate-800 dark:text-purple-400 dark:border-slate-700'
        ...
        {signedPdfBytes && (
          <div className="mt-4 space-y-3">
            ...
            {saveError && (
              <p className="text-sm text-red-600 bg-red-500/10 border border-red-500/20 rounded-lg p-3 text-center dark:text-red-400">
                {saveError}
              </p>
            )}
          </div>
        ...
        <div className="flex items-center justify-center gap-2 mt-4 text-xs text-slate-600 dark:text-slate-500">
          <ShieldCheck className="w-4 h-4 text-green-600 dark:text-green-400" /> Secure client-side processing. Your files never leave your device.
        </div>
      </div>
