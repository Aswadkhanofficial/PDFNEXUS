...
export default function AuthModal({ open, onClose, message = AI_GATE_MESSAGE }) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-md bg-slate-900/95 border border-slate-700/60 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in duration-200"
      ...
      >
        ...
        <div className="relative p-8 flex flex-col items-center text-center gap-4">
          <span className="w-16 h-16 bg-purple-500/20 text-purple-400 rounded-full flex items-center justify-center">
            <Sparkles className="w-8 h-8" />
          </span>
          <h2 className="text-2xl font-extrabold text-white">Create a free account</h2>
          ...
          <Link
            to="/login"
            onClick={onClose}
            className="w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-3.5 rounded-xl transition-colors"
          >
            Log in
          </Link>
        </div>
